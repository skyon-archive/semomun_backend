...
- vid(외래키): 문제가 속한 뷰어의 vid입니다.

- sid(외래키): 문제가 속한 섹션의 wid입니다.
...
- content: 문제의 내용입니다. 256바이트 이하의 가변 길이 문자열입니다.
  ...
  - 해당 값이 i일 경우 DB의 `/images/content/AxA/i` 에 해당하는 AxA 크기의 문제 이미지가 존재해야 합니다.

- explanation: 문제의 해설입니다. 256바이트 이하의 가변 길이 문자열입니다.

  - 해당 값이 i일 경우 DB의 `/images/explanation/AxA/i` 에 해당하는 AxA 크기의 문제 이미지가 존재해야 합니다.
...
- material: 뷰어의 자료로 쓰일 이미지 정보입니다. 4바이트의 음이 아닌 정수입니다.

  - 해당 값이 i일 경우 DB의 `/tmp/view/i.png`에 해당하는 뷰어 이미지가 존재해야 합니다.
...
- image: 섹션의 표지 이미지입니다. 256바이트 이하의 가변 길이 문자열입니다.
...
- image: 문제집의 표지 이미지입니다. 256바이트 이하의 가변 길이 문자열입니다.
